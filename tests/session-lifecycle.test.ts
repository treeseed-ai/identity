...
async function fixture({ subject = 'user', idToken = true, rejected = false, revocationEndpoint = `${issuer}/revoke`, audience = 'https://api.example.test', clientId = 'admin' } = {}) {
  const calls: string[] = [];
  const transport: typeof fetch = async (url, init) => {
    ...
    calls.push(String(url)); const body = new URLSearchParams(init.body as string);
    ...
    const access = await new SignJWT({ typ: 'Bearer', azp: clientId, scope: 'treeseed:read' }).setProtectedHeader({ alg: 'RS256' })
      .setIssuer(issuer).setAudience(audience).setSubject(subject).setIssuedAt().setExpirationTime('1m').sign(pair.privateKey);
    return Response.json({ access_token: access, refresh_token: 'refresh-new', token_type: 'Bearer', expires_in: 60, ...(idToken ? { id_token: token } : {}) });
  };
  const client = await createBrowserOidcClient({ issuer, clientId: 'admin', redirectUri: 'https://admin.example.test/callback', privateKey: pair.privateKey,
    ...
    resource: 'https://api.example.test', scopes: ['treeseed:read'], profile: 'keycloak', verificationKey: pair.publicKey,
...
test('refresh rejects a changed subject or issuer without disclosing tokens', async () => {
  ...
  await assert.rejects(client.refresh('refresh', { issuer, subject: 'user' }), { code: 'identity_authentication_failed' });
});
test('revocation uses the client-bound endpoint and rejects provider errors cleanly', async () => {
  ...
  await assert.rejects(client.revoke('refresh'), error => error instanceof Error && !error.message.includes('private-provider-detail'));
  await assert.rejects(fixture({ revocationEndpoint: 'https://attacker.test/revoke' }));
});
test('refresh without an ID token still checks access subject, client and audience', async () => {
  for (const options of [{ subject: 'other' }, { audience: 'https://other-market.example.test' }, { clientId: 'other' }]) {
    const { client } = await fixture({ ...options, idToken: false });
    await assert.rejects(client.refresh('refresh', { issuer, subject: 'user' }), { code: 'identity_authentication_failed' });
  }
});
